...
class DBLoggingMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        """Clears existing queries at the start of the request to ensure only
        queries from the current request are logged."""
        ...
        for alias in connections:
            ...
            if conn.queries:
                ...
                for query in conn.queries:
                    ...
                    logger.debug(f"[{alias}] {operation} | Duration: {duration:.3f}s | User: {username}\n{sql}")
        return response
...
def get_user(token):
    """Given a JWT token, return the associated User or AnonymousUser if
    invalid."""
    try:
...
class TokenAuthMiddleware(BaseMiddleware):
    """Custom Channels middleware that extracts a JWT from the 'Authorization'
    header and populates scope['user'] with the corresponding Django user."""
